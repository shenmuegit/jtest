...
public class BackUpChildAdapter extends BaseQuickAdapter<RemoteFile, BaseViewHolder> {

    private Context mContext;
    private BackUpParentAdapter parentAdapter;
    private int itemLayout;//按时间排序，共两种布局子布局，列表布局和网格布局

    public BackUpChildAdapter(Context context, int itemLayout, List<RemoteFile> data,
                              BackUpParentAdapter parentAdapter) {
    ...
    protected void convert(BaseViewHolder helper, RemoteFile item) {
        ...
        switch (itemLayout) {
            ...
            case R.layout.item_file_list:
                if (parentAdapter.isSelectMode()) {
                    helper.setGone(R.id.cbSelectMode, true).setGone(R.id.ivNormalMode, false);
                    if (parentAdapter.isChecked(item)) {
                        helper.setImageResource(R.id.cbSelectMode, R.drawable.icon_checked);
                    } else {
                        helper.setImageResource(R.id.cbSelectMode, R.drawable.icon_no_checked);
                    ...
                    helper.setGone(R.id.cbSelectMode, false).setGone(R.id.ivNormalMode, true);
                }
